...
const symbols = [
    "graficos/campana.svg",  // 🔔 Campana
    "graficos/bola.svg",     // 🎄 Bola navideña
    "graficos/regalo.svg",   // 🎁 Regalo
    "graficos/arbol.svg",    // 🎄 Árbol
    "graficos/estrella.svg", // ⭐ Estrella
    "graficos/santa.svg"     // 🎅 Santa Claus - JACKPOT
];

// Multiplicadores base (se multiplican por la apuesta)
const prizeMultipliers = {
    "graficos/campana.svg": 2,
    "graficos/bola.svg": 5,
    "graficos/regalo.svg": 10,
    "graficos/arbol.svg": 25,
...
function animateSlots(targetSymbols, callback) {
    ...
    // Añadir clase spinning a todas
    slotElements.forEach(slot => slot.classList.add("spinning"));
    
    let iterations = 15; // Todas las celdas giran juntas (más rápido y fluido)
    const spinDuration = 60; // Optimizado para 60fps
    ...
    const allInterval = setInterval(() => {
        ...
        // Girar todas las celdas a la vez (más fluido)
        slotInners.forEach(inner => {
        ...
        if (--iterations <= 0) {
            clearInterval(allInterval);
            // Mostrar resultado final en todas las celdas
    ...
    activeIntervals.push(allInterval);
}
...
function spinSlot(cost, spinButton) {
    ...
    // Descontar crédito INMEDIATAMENTE UNA SOLA VEZ
    ...
    // Sistema de probabilidades ponderadas (difícil ganar)
    const results = [];
    for (let i = 0; i < 15; i++) {
        const random = Math.random() * 100;
        let symbol;
        if (random < 40) symbol = symbols[0];      // Campana 40%
        else if (random < 65) symbol = symbols[1]; // Bola 25%
        else if (random < 82) symbol = symbols[2]; // Regalo 17%
        else if (random < 92) symbol = symbols[3]; // Árbol 10%
        else if (random < 97) symbol = symbols[4]; // Estrella 5%
        else symbol = symbols[5];                   // Santa 3%
        results.push(symbol);
    }
    ...
    animateSlots(results, () => {
        ...
        lineas.forEach(linea => {
            // Contar símbolos iguales consecutivos (mínimo 3)
            const firstSymbol = results[linea[0]];
            let consecutivos = 1;
            
            for (let i = 1; i < linea.length; i++) {
                if (results[linea[i]] === firstSymbol) {
                    consecutivos++;
                } else {
                    break;
                }
            ...
            // Si hay 3 o más símbolos iguales consecutivos
            if (consecutivos >= 3) {
                hayPremio = true;
                
                // Añadir solo los símbolos ganadores
                for (let i = 0; i < consecutivos; i++) {
                    winningIndices.push(linea[i]);
                }
                
                // Calcular premio usando multiplicadores
                const symbolMultiplier = prizeMultipliers[firstSymbol] || 0;
                // Bonificación por símbolos extra: 3=x1, 4=x2, 5=x3
                const bonusFactor = consecutivos - 2;
                premioTotal += symbolMultiplier * bonusFactor;
                
                // Detectar jackpot
                if (firstSymbol === "graficos/santa.svg") {
                    isJackpot = true;
                }
            }
        });
